...
export function buildReactCode(state: SettingsPanelState) {
  ...
  "function buildShadow(s) { if (!s.shadowEnabled) return \"none\"; var hex = Math.round(s.shadowOpacity * 255).toString(16).padStart(2, \"0\"); return s.shadowX + \"px \" + s.shadowY + \"px \" + s.shadowBlur + \"px \" + s.shadowSpread + \"px \" + s.shadowColor + hex; }",
    ...
    "  };",
    "  const groupStyle = { display: \"grid\", gap: 12, margin: 0, padding: 14, border: `1px solid ${state.border}`, borderRadius: Math.max(12, state.radius - 8), transition: state.transitionDuration > 0 ? \"opacity 0.2s ease, border-color 0.2s ease\" : \"none\" };",
    ...
    "            }",
    "            return (",
    "              <label key={field} htmlFor={fieldId} style={{ display: \"flex\", alignItems: \"center\", justifyContent: \"space-between\", gap: 12, fontSize: state.bodySize, transition: state.transitionDuration > 0 ? \"background 0.15s ease, color 0.15s ease\" : \"none\" }}>",
    "                <span>",
    "                  <strong>{field}</strong>",
    ...
    "        <button type=\"submit\" disabled={state.disabled || state.previewState === \"loading\"} style={{ border: 0, borderRadius: 12, padding: \"10px 14px\", background: state.accent, color: state.actionText, fontWeight: 700, transition: state.transitionDuration > 0 ? \"background 0.15s ease, opacity 0.15s ease\" : \"none\" }}>Save settings</button>",
    "        {state.showReset && <button type=\"reset\" disabled={state.disabled} style={{ border: `1px solid ${state.border}`, borderRadius: 12, padding: \"10px 14px\", background: \"transparent\", color: state.foreground, transition: state.transitionDuration > 0 ? \"border-color 0.15s ease, color 0.15s ease\" : \"none\" }}>Reset settings</button>}",
    "      </div>",
    "    </form>",
    "  );",
    "}",
