...
export function buildReactCode(state: SettingsPanelState) {
  return [
    ...
    "  };",
    "  const groupStyle = { display: \"grid\", gap: 12, margin: 0, padding: 14, border: `1px solid ${state.border}`, borderRadius: Math.max(12, state.radius - 8) };",
    ...
    "            }",
    "            return (",
    "              <label key={field} htmlFor={fieldId} style={{ display: \"flex\", alignItems: \"center\", justifyContent: \"space-between\", gap: 12, fontSize: state.bodySize }}>",
    "                <span>",
    "                  <strong>{field}</strong>",
    ...
    "        <button type=\"submit\" disabled={state.disabled || state.previewState === \"loading\"} style={{ border: 0, borderRadius: 12, padding: \"10px 14px\", background: state.accent, color: \"#020617\", fontWeight: 700 }}>Save settings</button>",
    "        {state.showReset && <button type=\"reset\" disabled={state.disabled} style={{ border: `1px solid ${state.border}`, borderRadius: 12, padding: \"10px 14px\", background: \"transparent\", color: state.foreground }}>Reset settings</button>}",
    "      </div>",
    "    </form>",
    "  );",
    "}",
