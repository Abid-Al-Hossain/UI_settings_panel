...
export function buildReactCode(state: SettingsPanelState) {
  return [
    ...
    "        <button type=\"submit\" disabled={state.disabled || state.previewState === \"loading\"} style={{ border: 0, borderRadius: 12, padding: \"10px 14px\", background: state.accent, color: \"#020617\", fontWeight: 700, transition: state.motion ? \"background 0.15s ease, opacity 0.15s ease\" : \"none\" }}>Save settings</button>",
